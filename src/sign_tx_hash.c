...
UX_STEP_NOCB(ux_transfer_esdt_flow_24_step, bnnn_paging,
             {
                 .title = "Token",
                 .text = esdt_info.ticker,
             });
UX_STEP_NOCB(ux_transfer_esdt_flow_25_step, bnnn_paging,
             {
                 .title = "Value",
                 .text = tx_context.amount,
             });
UX_STEP_NOCB(ux_transfer_esdt_flow_26_step, bnnn_paging,
             {
                 .title = "Receiver",
                 .text = tx_context.receiver,
             });
UX_STEP_NOCB(ux_transfer_esdt_flow_27_step, bnnn_paging,
             {
                 .title = "Fee",
                 .text = tx_context.fee,
             });
UX_STEP_NOCB(ux_transfer_esdt_flow_28_step, bnnn_paging,
             {
                 .title = "Network",
                 .text = tx_context.network,
             });
UX_STEP_VALID(ux_transfer_esdt_flow_29_step, pb,
              send_response(set_result_signature(), true),
              {
                  &C_icon_validate_14,
                  "Confirm transfer",
              });
UX_STEP_VALID(ux_transfer_esdt_flow_30_step, pb, send_response(0, false),
              {
                  &C_icon_crossmark,
                  "Reject",
              });

UX_FLOW(ux_transfer_esdt_flow, &ux_transfer_esdt_flow_24_step,
        &ux_transfer_esdt_flow_25_step, &ux_transfer_esdt_flow_26_step,
        &ux_transfer_esdt_flow_27_step, &ux_transfer_esdt_flow_28_step,
        &ux_transfer_esdt_flow_29_step, &ux_transfer_esdt_flow_30_step);

// UI for confirming the tx details of the transaction on screen
UX_STEP_NOCB(ux_sign_tx_hash_flow_17_step, bnnn_paging,
             {
                 .title = "Receiver",
                 .text = tx_context.receiver,
             });
UX_STEP_NOCB(ux_sign_tx_hash_flow_18_step, bnnn_paging,
             {
                 .title = "Amount",
                 .text = tx_context.amount,
             });
UX_STEP_NOCB(ux_sign_tx_hash_flow_19_step, bnnn_paging,
             {
                 .title = "Fee",
                 .text = tx_context.fee,
             });
UX_STEP_NOCB(ux_sign_tx_hash_flow_20_step, bnnn_paging,
             {
                 .title = "Data",
                 .text = tx_context.data,
             });
UX_STEP_NOCB(ux_sign_tx_hash_flow_21_step, bnnn_paging,
             {
                 .title = "Network",
                 .text = tx_context.network,
             });
UX_STEP_VALID(ux_sign_tx_hash_flow_22_step, pb,
              send_response(set_result_signature(), true),
              {
                  &C_icon_validate_14,
                  "Sign transaction",
              });
UX_STEP_VALID(ux_sign_tx_hash_flow_23_step, pb, send_response(0, false),
              {
                  &C_icon_crossmark,
                  "Reject",
              });
...
bool sign_tx_hash(uint8_t *data_buffer) {
  ...
  BEGIN_TRY {
    TRY {
      cx_hash((cx_hash_t *)&sha3_context, CX_LAST, data_buffer, 0,
              tx_hash_context.hash, 32);
      cx_eddsa_sign(&private_key, CX_RND_RFC6979 | CX_LAST, CX_SHA512,
                    tx_hash_context.hash, 32, NULL, 0, tx_context.signature, 64,
                    NULL);
    }
    CATCH_ALL { success = false; }
    FINALLY { explicit_bzero(&private_key, sizeof(private_key)); }
  }
  END_TRY;
...
void handle_sign_tx_hash(uint8_t p1, uint8_t *data_buffer, uint16_t data_length,
  ...
  if (is_esdt_transfer()) {
    uint16_t res;
    res =
        parse_esdt_data(tx_context.data, tx_context.data_size + DATA_SIZE_LEN);
    if (res != MSG_OK) {
      THROW(res);
    }
...
bool is_esdt_transfer() {
  ...
  if (has_data && identifier_len_valid) {
    has_esdt_transfer_prefix =
        strncmp(tx_context.data + DATA_SIZE_LEN - 1, ESDT_TRANSFER_PREFIX,
                ESDT_TRANSFER_PREFIX_LENGTH) == 0;
    has_registered_esdt_identifier =
        strncmp(tx_context.data + DATA_SIZE_LEN - 1 +
                    ESDT_TRANSFER_PREFIX_LENGTH,
                esdt_info.identifier, esdt_info.identifier_len) == 0;

    size_t identifier_end_position = DATA_SIZE_LEN - 1 +
                                     ESDT_TRANSFER_PREFIX_LENGTH +
                                     esdt_info.identifier_len;
