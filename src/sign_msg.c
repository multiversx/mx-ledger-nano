...
UX_STEP_NOCB(ux_sign_msg_flow_14_step, bnnn_paging,
             {
                 .title = "Hash",
                 .text = msg_context.strhash,
             });
UX_STEP_VALID(ux_sign_msg_flow_15_step, pb,
              send_response(set_result_signature(), true),
              {
                  &C_icon_validate_14,
                  "Sign message",
              });
UX_STEP_VALID(ux_sign_msg_flow_16_step, pb, send_response(0, false),
              {
                  &C_icon_crossmark,
                  "Reject",
              });

UX_FLOW(ux_sign_msg_flow, &ux_sign_msg_flow_14_step, &ux_sign_msg_flow_15_step,
  ...
  BEGIN_TRY {
    TRY {
      cx_eddsa_sign(&private_key, CX_RND_RFC6979 | CX_LAST, CX_SHA512,
                    msg_context.hash, HASH_LEN, NULL, 0, msg_context.signature,
                    MESSAGE_SIGNATURE_LEN, NULL);
    }
...
void handle_sign_msg(uint8_t p1, uint8_t *data_buffer, uint16_t data_length,
 ...
 */
  if (p1 == P1_FIRST) {
    ...
    cx_hash((cx_hash_t *)&sha3_context, 0, (uint8_t *)message_length_str,
            strlen(message_length_str), NULL, 0);
  } else {
    if (p1 != P1_MORE) {
  ...
  cx_hash((cx_hash_t *)&sha3_context, CX_LAST, data_buffer, 0, msg_context.hash,
          HASH_LEN);
  snprintf(msg_context.strhash, sizeof(msg_context.strhash), "%.*H",
           sizeof(msg_context.hash), msg_context.hash);
