...
UX_STEP_NOCB(ux_auth_token_msg_flow_33_step, bnnn_paging,
             {
                 .title = "Address",
                 .text = token_auth_context.address,
             });
UX_STEP_NOCB(ux_auth_token_msg_flow_34_step, bnnn_paging,
             {
                 .title = "Auth Token",
                 .text = token_auth_context.token,
             });
UX_STEP_VALID(ux_auth_token_msg_flow_35_step, pb,
              send_response(set_result_auth_token(), true),
              {
                  &C_icon_validate_14,
                  "Authorize",
              });
UX_STEP_VALID(ux_auth_token_msg_flow_36_step, pb, send_response(0, false),
              {
                  &C_icon_crossmark,
                  "Reject",
              });

UX_FLOW(ux_auth_token_msg_flow, &ux_auth_token_msg_flow_33_step,
        &ux_auth_token_msg_flow_34_step, &ux_auth_token_msg_flow_35_step,
...
void update_token_display_data(uint8_t const *data_buffer,
...
static uint8_t set_result_auth_token(void) {
  ...
  memmove(complete_response, token_auth_context.address,
          strlen(token_auth_context.address));
  memmove(complete_response + strlen(token_auth_context.address),
          token_auth_context.signature, MESSAGE_SIGNATURE_LEN);
  const uint8_t response_size =
      strlen(token_auth_context.address) + MESSAGE_SIGNATURE_LEN;
  ...
  BEGIN_TRY {
    TRY {
      cx_eddsa_sign(&private_key, CX_RND_RFC6979 | CX_LAST, CX_SHA512,
                    token_auth_context.hash, HASH_LEN, NULL, 0,
                    token_auth_context.signature, MESSAGE_SIGNATURE_LEN, NULL);
    }
...
void handle_auth_token(uint8_t p1, uint8_t *data_buffer, uint16_t data_length,
  ...
  */
  if (p1 == P1_FIRST) {
    ...
    uint32_t const address_index =
        read_uint32_be(data_buffer + sizeof(uint32_t));
    ...
    cx_hash((cx_hash_t *)&sha3_context, 0, (uint8_t *)PREPEND,
            sizeof(PREPEND) - 1, NULL, 0);
    ...
    cx_hash((cx_hash_t *)&sha3_context, 0, (uint8_t *)token_length_str,
            strlen(token_length_str), NULL, 0);

    // add the message length to the hash
    cx_hash((cx_hash_t *)&sha3_context, 0,
            (uint8_t *)token_auth_context.address,
            strlen(token_auth_context.address), NULL, 0);
  } else {
    if (p1 != P1_MORE) {
  ...
  cx_hash((cx_hash_t *)&sha3_context, CX_LAST, data_buffer, 0,
          token_auth_context.hash, HASH_LEN);
