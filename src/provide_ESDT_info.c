...
static bool verify_signature(const uint8_t *data_buffer, uint16_t data_length,
                             size_t required_len) {
  ...
  cx_ecfp_init_public_key(CX_CURVE_256K1, LEDGER_SIGNATURE_PUBLIC_KEY,
                          sizeof(LEDGER_SIGNATURE_PUBLIC_KEY), &tokenKey);

  int signature_size = data_length - required_len;
  return cx_ecdsa_verify(&tokenKey, CX_LAST, CX_SHA256, hash, 32,
                         data_buffer + required_len, signature_size);
}
#endif
...
uint16_t handle_provide_ESDT_info(const uint8_t *data_buffer,
  ...
  if (esdt_info->ticker_len >= sizeof(esdt_info->ticker) ||
      data_length < required_len) {
    return ERR_MESSAGE_INCOMPLETE;
  }
  memcpy(esdt_info->ticker, data_buffer + last_required_len,
         esdt_info->ticker_len);
  ...
  if (esdt_info->identifier_len >= sizeof(esdt_info->identifier) ||
      data_length < required_len) {
    return ERR_MESSAGE_INCOMPLETE;
  }
  memcpy(esdt_info->identifier, data_buffer + last_required_len,
  ...
  if (esdt_info->chain_id_len >= sizeof(esdt_info->chain_id) ||
      data_length < required_len) {
    return ERR_MESSAGE_INCOMPLETE;
  }
  memcpy(esdt_info->chain_id, data_buffer + last_required_len,
         esdt_info->chain_id_len);
