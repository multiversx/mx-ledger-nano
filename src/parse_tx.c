...
// ticker name
bool make_amount_pretty(char *amount, size_t max_size, char *ticker,
                        int decimals_places) {
...
bool gas_to_fee(uint64_t gas_limit, uint64_t gas_price, uint32_t data_size,
                char *fee, size_t size) {
...
uint16_t verify_value(bool *valid) {
  if (strncmp(tx_hash_context.current_field, VALUE_FIELD,
              strlen(VALUE_FIELD)) == 0) {
    if (tx_hash_context.current_value_len >= sizeof(tx_context.amount)) {
      return ERR_AMOUNT_TOO_LONG;
    }
    if (!valid_amount(tx_hash_context.current_value,
                      strlen(tx_hash_context.current_value))) {
      return ERR_INVALID_AMOUNT;
    }
    memmove(tx_context.amount, tx_hash_context.current_value,
...
uint16_t verify_receiver(bool *valid) {
  if (strncmp(tx_hash_context.current_field, RECEIVER_FIELD,
              strlen(RECEIVER_FIELD)) == 0) {
    if (tx_hash_context.current_value_len >= sizeof(tx_context.receiver)) {
      return ERR_RECEIVER_TOO_LONG;
    }
    memmove(tx_context.receiver, tx_hash_context.current_value,
            tx_hash_context.current_value_len);
    *valid = true;
  }
  return MSG_OK;
...
uint16_t verify_gasprice(bool *valid) {
  if (strncmp(tx_hash_context.current_field, GASPRICE_FIELD,
              strlen(GASPRICE_FIELD)) == 0) {
...
uint16_t verify_gaslimit(bool *valid) {
  if (strncmp(tx_hash_context.current_field, GASLIMIT_FIELD,
              strlen(GASLIMIT_FIELD)) == 0) {
    if (!parse_int(tx_hash_context.current_value,
                   strlen(tx_hash_context.current_value),
...
uint16_t verify_data(bool *valid) {
  if (strncmp(tx_hash_context.current_field, DATA_FIELD, strlen(DATA_FIELD)) ==
      0) {
#ifndef FUZZING
...
#endif
    tx_hash_context.current_value_len =
        tx_hash_context.current_value_len / 4 * 4;
    ...
    if (ascii_len > MAX_DISPLAY_DATA_SIZE) {
      ...
      memmove(encoded + MAX_DISPLAY_DATA_SIZE - ellipsisLen, ellipsis,
              ellipsisLen);
    }
    if (!base64decode(tx_context.data, encoded, ascii_len)) {
      return ERR_INVALID_MESSAGE;
    }
    if (strncmp(tx_context.data, ESDT_TRANSFER_PREFIX,
                ESDT_TRANSFER_PREFIX_LENGTH) == 0) {
      extract_esdt_value(tx_hash_context.current_value,
                         tx_hash_context.current_value_len);
    }
...
static void extract_esdt_value(const uint8_t *encoded_data_field,
  ...
  for (int idx = esdt_value_start_position; idx < strlen(data_field); idx++) {
    if (data_field[idx] == SC_ARGS_SEPARATOR ||
        data_field[idx] == BASE_64_INVALID_CHAR) {
  ...
  if (esdt_value_end_position - esdt_value_start_position + 1 >
      MAX_ESDT_VALUE_HEX_COUNT) {
  ...
  size_t num_chars_to_copy =
      esdt_value_end_position - esdt_value_start_position + 1;
  memmove(tx_context.esdt_value + 1, data_field + esdt_value_start_position,
          num_chars_to_copy);
...
uint16_t verify_chainid(bool *valid) {
  if (strncmp(tx_hash_context.current_field, CHAINID_FIELD,
              strlen(CHAINID_FIELD)) == 0) {
    char *ticker = TICKER_TESTNET;
    if (strncmp(tx_hash_context.current_value, MAINNET_CHAIN_ID,
                strlen(MAINNET_CHAIN_ID)) == 0) {
      ticker = TICKER_MAINNET;
    }
    memmove(tx_context.chain_id, tx_hash_context.current_value,
            tx_hash_context.current_value_len);
    set_network(tx_hash_context.current_value);

    if (!gas_to_fee(tx_context.gas_limit, tx_context.gas_price,
                    tx_context.data_size, tx_context.fee,
                    sizeof(tx_context.fee) - PRETTY_SIZE)) {
      return ERR_INVALID_FEE;
    }

    if (!make_amount_pretty(tx_context.amount, sizeof(tx_context.amount),
                            ticker, DECIMAL_PLACES) ||
        !make_amount_pretty(tx_context.fee, sizeof(tx_context.fee), ticker,
                            DECIMAL_PLACES)) {
...
uint16_t verify_version(bool *valid) {
  if (strncmp(tx_hash_context.current_field, VERSION_FIELD,
              strlen(VERSION_FIELD)) == 0) {
    uint64_t version;
    if (!parse_int(tx_hash_context.current_value,
                   strlen(tx_hash_context.current_value), &version)) {
      return ERR_INVALID_MESSAGE;
    }
...
uint16_t verify_options(bool *valid) {
  if (strncmp(tx_hash_context.current_field, OPTIONS_FIELD,
              strlen(OPTIONS_FIELD)) == 0) {
    uint64_t options;
    if (!parse_int(tx_hash_context.current_value,
                   strlen(tx_hash_context.current_value), &options)) {
      return ERR_INVALID_MESSAGE;
    }
...
uint16_t process_field(void) {
  if (tx_hash_context.current_field_len == 0 ||
  ...
  valid_field |= strncmp(tx_hash_context.current_field, NONCE_FIELD,
                         strlen(NONCE_FIELD)) == 0;
  valid_field |= strncmp(tx_hash_context.current_field, SENDER_FIELD,
                         strlen(SENDER_FIELD)) == 0;
  valid_field |= strncmp(tx_hash_context.current_field, SENDER_USERNAME_FIELD,
                         strlen(SENDER_USERNAME_FIELD)) == 0;
  valid_field |= strncmp(tx_hash_context.current_field, RECEIVER_USERNAME_FIELD,
...
uint16_t parse_data(const uint8_t *data_buffer, uint16_t data_length) {
  ...
  for (;;) {
    ...
    case JSON_PROCESSING_STRING_VALUE: {
      bool is_data_field = strncmp(tx_hash_context.current_field, DATA_FIELD,
                                   tx_hash_context.current_field_len) == 0;
      if (c == '"') {
        if (is_data_field) {
...
uint16_t parse_esdt_data() {
  ...
  res = parse_hex(tx_context.esdt_value + 1, strlen(tx_context.esdt_value) - 1,
                  &value);
  if (!res) {
  ...
  if (!make_amount_pretty(amount,
                          ...
                          esdt_info.ticker, esdt_info.decimals)) {
    return ERR_PRETTY_FAILED;
  }
